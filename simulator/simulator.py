...
BUOYS = [
    ...
    ("70b3d57ed0060001", "BY-01", 8.12, 26.6, 7.1, 12.4, 20.2, 286, 0.04, True),
    ("70b3d57ed0060002", "BY-02", 7.90, 26.3, 6.8, 12.0, 19.7, 250, 0.03, True),
    ("70b3d57ed0060003", "BY-03", 8.24, 25.6, 7.4, 6.2, 20.7, 312, 0.02, True),
    ("70b3d57ed0060004", "BY-04", 8.10, 25.7, 7.0, 10.0, 20.1, 280, 0.03, False),
    ("70b3d57ed0060005", "BY-05", 8.18, 25.7, 7.2, 9.5, 20.4, 299, 0.01, True),
    ("70b3d57ed0060006", "BY-06", 8.09, 26.9, 7.0, 7.8, 20.0, 278, 0.03, True),
    ("70b3d57ed0060007", "BY-07", 7.71, 24.8, 4.7, 31.2, 19.5, 205, 0.12, True),
    ("70b3d57ed0060008", "BY-08", 8.21, 27.3, 7.5, 5.4, 20.9, 320, 0.01, True),
    ("70b3d57ed0060009", "BY-09", 8.15, 26.4, 7.0, 8.2, 20.5, 291, 0.03, True),
    ("70b3d57ed006000a", "BY-10", 8.07, 26.1, 6.9, 11.0, 20.1, 275, 0.04, True),
    ("70b3d57ed006000b", "BY-11", 7.89, 25.1, 5.4, 18.5, 19.8, 242, 0.07, True),
    ("70b3d57ed006000c", "BY-12", 8.22, 27.0, 7.3, 7.0, 20.6, 308, 0.02, True),
    ("70b3d57ed006000d", "BY-13", 8.11, 24.5, 7.1, 9.8, 20.3, 284, 0.03, True),
    ("70b3d57ed006000e", "BY-14", 8.19, 27.5, 7.4, 4.8, 20.8, 315, 0.01, True),
    ("70b3d57ed006000f", "BY-15", 8.28, 24.9, 7.6, 5.2, 21.0, 328, 0.01, True),
    ("70b3d57ed0060010", "BY-16", 8.14, 25.3, 7.1, 8.9, 20.4, 290, 0.02, False),
]
...
class Buoy:
    ...
    def step(self, tick: int, spill_enabled: bool) -> None:
        ...
        self.temp = self._walk(self.temp + daily * 0.02, 0.05, 22.0, 30.0)
        self.do = self._walk(self.do, 0.06, 2.5, 8.2)
        self.turb = self._walk(self.turb, 0.4, 1.0, 120.0)
        self.ec = self._walk(self.ec, 0.03, 18.5, 22.0)
    ...
    def _apply_spill(self, tick: int) -> None:
        ...
        self.do = round(max(2.2, 6.8 - curve * 4.0), 2)
